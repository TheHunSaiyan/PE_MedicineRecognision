...
class CalibrationSettings:
    def __init__(self, calibration_manager):
        """
        Initialize the CalibrationSettings with a calibration manager.

        Args: 
            calibration_manager (CalibrationManager): The manager for calibration operations.
    ...
    async def get_camera_calibration_settings(self):
        """
        Retrieve the current camera calibration parameters.

        Args:
            None

        Returns:
            Dict[str, Any]: A dictionary containing the current camera calibration parameters.
        """
    ...
    async def update_camera_calibration_parameters(self, params):
        """
        Update the camera calibration parameters with the provided values.

        Args:
            params (CalibrationParameters): The new calibration parameters to apply.
    ...
    async def upload_calibration_images(self, files):
        ...
        Args:
            files (List[UploadFile]): List of uploaded files containing calibration images.

        Returns:
            FileResponse: A response containing the calibration results.
        """
        try:
            ...
            for file in files:
                try:
                    ...
                    file_path = os.path.join(
                        AppConfig.CALIBRATION_IMAGES_DIR, filename)
                    ...
                    ret, corners = cv2.findChessboardCorners(
                        grey,
                        (self.calibration_manager.calibration_params.chess_col,
                         self.calibration_manager.calibration_params.chess_row),
                        None,
            ...
            if good_files == 0:
                logger.error(
                    "No valid calibration images found. Could not detect chessboard in any image.")
                raise HTTPException(
                    status_code=400,
                    detail="No valid calibration images found. Could not detect chessboard in any image."
                )

            objp = np.zeros((self.calibration_manager.calibration_params.chess_row *
                            self.calibration_manager.calibration_params.chess_col, 3), np.float32)
            objp[:, :2] = np.mgrid[0:self.calibration_manager.calibration_params.chess_col,
                                   0:self.calibration_manager.calibration_params.chess_row].T.reshape(-1, 2)
    ...
    async def generate_new_matrix_file(self):
        """
        Generate a new camera matrix file based on the current calibration data.

        Args:
            None

        Returns:
            FileResponse: A response containing the new camera matrix file.
        """
        try:
            calibration_data = np.load(
                "Data/NPZ/camera_calibration_params.npz", allow_pickle=True)
            ...
            for key in required_keys:
                if key not in calibration_data:
                    logger.error(
                        f"Missing required data in calibration file: {key}")
                    raise HTTPException(
                        status_code=400,
            ...
            dist_coefficients = np.ascontiguousarray(
                calibration_data['distortion_coefficients'])
            object_points = np.ascontiguousarray(
                calibration_data['object_points'])
            rotation_vectors = np.ascontiguousarray(
                calibration_data['rotation_vectors'])
            translation_vectors = np.ascontiguousarray(
                calibration_data['translation_vectors'])
            ...
            if first_image is None:
                logger.error(
                    f"Could not read first calibration image at {first_image_path}")
                raise HTTPException(
                    status_code=400,
            ...
            if valid_projections == 0:
                logger.error(
                    "Could not calculate reprojection error - no valid projections")
                raise HTTPException(
                    status_code=500,
            ...
            if errors > self.calibration_manager.calibration_params.error_threshold:
                logger.error(
                    f"Calibration error too high: {errors:.2f}. Please check your images.")
                raise HTTPException(
                    status_code=500,
    ...
    async def undistort_images(self):
        """
        Undistort images using the saved camera matrix and distortion coefficients.
        ...
        try:
            ...
            dist_coefficients = np.ascontiguousarray(
                calibration_data['distortion_coefficients'])
            undistorted_camera_matrix = np.ascontiguousarray(
                calibration_data['undistorted_camera_matrix'])
    ...
    async def upload_camera_calibtration(self, file):
        """
        Upload a camera calibration file and save the parameters.

        Args:
            file (UploadFile): The uploaded file containing calibration data.
    ...
    async def upload_undistorted(self, file):
        """
        Upload a file containing undistorted camera matrix data and save it.

        Args:
            file (UploadFile): The uploaded file containing undistorted matrix data.
        ...
        try:
            ...
            for key in required_keys:
                if key not in calibration_data:
                    logger.error(
                        f"Missing required data in undistorted matrix file: {key}")
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
            ...
            np.savez(
                ...
                object_points=calibration_data['object_points'],
                image_points=calibration_data['image_points'],
                original_image_paths=calibration_data.get(
                    'original_image_paths', [])
            )
            ...
            logger.error(
                f"Failed to process undistorted matrix file: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
