...
interface DataAvailability {
  val_images: boolean;
  val_segmentation_labels: boolean;
  val_mask_images: boolean;
  train_images: boolean;
  train_segmentation_labels: boolean;
  train_mask_images: boolean;
}
...
const [availability, setAvailability] = useState<DataAvailability>({
    val_images: false,
    val_segmentation_labels: false,
    val_mask_images: false,
    train_images: false,
    train_mask_images: false,
 ...
 const [isComplete, setIsComplete] = useState(false);
  ...
  useEffect(() => {
    const fetchDataAvailability = async () => {
      try {
        const response = await fetch('http://localhost:2076/data_availability_for_augmentation');
        if (!response.ok) {
        ...
        setAvailability({
          ...
          val_segmentation_labels: data.val_segmentation_labels,
          val_mask_images: data.val_mask_images,
          train_images: data.train_images,
          train_segmentation_labels: data.train_segmentation_labels,
          train_mask_images: data.train_mask_images
        });
        setSuccess(true);
      } catch (err) {
