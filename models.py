...
class CameraCalibrationParameters(BaseModel):
    ...
    @classmethod
    def load_from_file(cls, filename: str = "camera_calibration_params.json") -> Optional['CameraCalibrationParameters']:
        if not os.path.exists(filename):
            return None
        try:
            with open(filename, 'r') as f:
    ...
    def save_to_file(self, filename: str = "camera_calibration_params.json"):
        with open(filename, 'w') as f:
            json.dump(self.dict(), f, indent=4)